...
exports.register = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            res.status(409).json({ status: 'failed', message: 'This email is already in use' });
        } else {
                                ...
                                <div style="background-color: white;">
                                    ...
                                    <div style="padding:1rem">
                                        ...
                                        <div style="text-align: center;">
                                            ...
                                            <a href="${process.env.FRONTEND_URL}/verify/${jwtToken}">${process.env.FRONTEND_URL}/verify/${jwtToken}</a>
                                        </div>
    ...
    console.log(`${process.env.FRONTEND_URL}/api/user/verify/${jwtToken}`);
}
...
exports.setPin = async(req,res) => {
    try {
        ...
        try {
            ...
            if (user) {
                res.json({ status: 'success', message: 'Pin updated.' });
                logger.info(`Pin updated for ${email}`);
            } else {
                res.status(404).json({ status: 'failed', message: 'User not found.' });
                logger.info(`Pin update failed for ${email} as user object is null.`);
            }
        } catch (err) {
            logger.error(`${err.name} ${err.message}`);
            res.status(500).json({ status: 'failed', message: `Some error occured ${err.message}` });
        ...
        logger.error(`Pin update failed ${err.name} ${err.message}`);
        res.status(500).json({ status: 'failed', message: `Some error occured ${err.message}` });
    }
...
exports.login = async(req,res) => {
    try{
        let user = await User.findOne({email:req.body.email, pin:req.body.pin});
        if(user){
            const jwtToken = jwt.sign({email: user.email, pin: user.pin}, jwtSecretKey, {expiresIn: '50h'});
            logger.info(`${user.email} logged in.`);
            res.json({status:'success', token: jwtToken});
        }else{
            logger.info(`${req.body.email} log in failed with message 'Email or password is wrong'.`);
            res.status(400).json({status:'failed', message: 'Email or password is wrong'});
        }
    }catch(err){
        logger.error(`Pin update failed ${err.name} ${err.message}`);
        res.status(500).json({ status: 'failed', message: `Some error occured ${err.message}` });
